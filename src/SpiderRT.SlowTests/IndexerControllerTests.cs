...
namespace SpiderRT.SlowTests
{
	public class IndexerControllerTests
	{
		...
		public void Should_pass_settings_and_vcs_roots_on_to_the_ingester()
		{
			...
			var indexController = new IndexerController(_documentStore, fakeIngester, A.Fake<IVcsManagerFactory>());

			indexController.RunIndexer();
		...
		public void Should_get_vcs_managers_for_each_stored_vcs_root_and_update_the_managers()
		{
			...
			var indexController = new IndexerController(_documentStore, A.Fake<IIngester>(), fakeVcsManagerFactory);
